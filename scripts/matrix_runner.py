...
from scripts.generate_matrix_configs import (  # type: ignore
    load_matrix_spec,
    write_configs,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an experiment matrix and log outcomes"
    )
    parser.add_argument("--matrix", required=True, help="Matrix spec toml path")
    parser.add_argument(
        "--regenerate",
        action="store_true",
        help="Force regeneration of config files/index",
    )
    parser.add_argument(
        "--resume",
        dest="resume",
        action="store_true",
        default=True,
        help="Skip configs already logged in results CSV (default)",
    )
    parser.add_argument(
        "--no-resume",
        dest="resume",
        action="store_false",
        help="Ignore previous CSV entries",
    )
    parser.add_argument(
        "--rerun-failed",
        action="store_true",
        help="Rerun entries that previously recorded non-success status",
    )
    parser.add_argument(
        "--limit",
        type=int,
        help="Optional cap on number of configs to execute this pass",
    )
    parser.add_argument(
        "--extra-args",
        default="",
        help="Extra CLI args forwarded to main.py (quoted string)",
    )
    parser.add_argument(
        "--python-bin", default=sys.executable, help="Python interpreter for main.py"
    )
    parser.add_argument(
        "--main-script", default="main.py", help="Entry script invoked per experiment"
    )
    parser.add_argument(
        "--program-config",
        default=None,
        help="program.toml override (falls back to matrix spec)",
    )
    parser.add_argument(
        "--output-root",
        default=None,
        help="Per-run artifact root override (falls back to matrix spec)",
    )
    parser.add_argument(
        "--results-csv",
        default=None,
        help="Results CSV override (falls back to matrix spec)",
    )
    parser.add_argument(
        "--progress-file",
        default=None,
        help="Optional JSON file to persist live batch progress",
    )
    parser.add_argument(
        "--heartbeat-sec",
        type=int,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Create run folders without executing main.py",
    )
    parser.add_argument(
        "--skip-artifact-copy",
        action="store_true",
        help="Do not copy figures/solutions into run folders",
    )
...
def read_last_nonempty_line(
    path: Path, max_bytes: int = 8192, max_chars: int = 180
) -> str:
    if not path.exists():
        return ""
    try:
...
def write_progress_snapshot(path: Path | None, payload: dict) -> None:
    if path is None:
        return
...
def snapshot_configs(
    instance_config: Path, program_config: Path, destination: Path
) -> None:
...
def run_experiment(
    ...
    heartbeat_sec = max(0, int(opts.heartbeat_sec))
    progress_path = Path(opts.progress_file) if opts.progress_file else None
    ...
    if opts.dry_run:
        ...
        start = time.time()
        print(f"[START {run_index}/{total_runs}] run_id={run_id}")
        with open(log_path, "w") as log_file:
            ...
            next_heartbeat = (
                start + heartbeat_sec if heartbeat_sec > 0 else float("inf")
            )
            while True:
                ...
                if now >= next_heartbeat:
                    elapsed = now - start
                    last_line = read_last_nonempty_line(log_path)
                    msg = (
                        f"[HEARTBEAT {run_index}/{total_runs}] run_id={run_id} "
                        f"elapsed={format_elapsed(elapsed)}"
                    )
                    if last_line:
                        msg += f" last_log='{last_line}'"
                    print(msg)
                    write_progress_snapshot(
                        progress_path,
                        {
        ...
        write_progress_snapshot(
            progress_path,
            {
                ...
                "elapsed_hhmmss": format_elapsed(duration),
                "config": str(config_path),
...
def main():
    ...
    results_csv = Path(
        args.results_csv or output_cfg.get("results_csv", "logs/matrix_results.csv")
    )
    progress_file = args.progress_file or str(
        Path("logs/repro") / f"{spec['matrix_id']}_progress.json"
    )
    program_config = args.program_config or spec.get(
        "program_config", "config/program.toml"
    )
    ...
    for run_index, entry in enumerate(pending_entries, start=1):
        metadata = run_experiment(
            batch_args,
            Path(entry["config"]),
            extra_main_args,
            run_index=run_index,
            total_runs=len(pending_entries),
        ...
        improvement = (
            metrics.get("improvement_percent", {}).get("over_baseline")
            if metrics
            else None
        )
        ...
        executed += 1
        print(
            f"[RUN {executed}/{len(pending_entries)}] {entry['config']} -> status={metadata['status']}"
        )

    print(
        f"Matrix sweep complete. Executed {executed} runs. Results appended to {results_csv}"
    )
