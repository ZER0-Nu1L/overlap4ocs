    ...
    if isinstance(source, list):  # NOTE: for "processors" model
        ...
        for processor in source:
            for task_name, start_time, finish_time in processor.tasks:
                schedule.append(
                    {
                        "step": int(task_name.replace("Step", ""))
        ...
        if solver == "gurobi":
            ...
            params["solver"] == "pulp"
            or params["solver"] == "copt"
            or params["solver"] == "pulp_gurobi"
        ):
            import pulp
...
def plot_schedule(
    ...
    for ocs in range(1, num_ocs + 1):
        ...
        for s in ocs_schedule:
            ...
            if s_offset["reconf"] > 0.5:
                ...
                ax.text(
                    s_offset["t_reconf_start"]
                    + (s_offset["t_reconf_end"] - s_offset["t_reconf_start"]) / 2,
                    y,
                    "Reconf",
                    ha="center",
                    va="center",
            ...
            if s_offset["used"] > 0.5 and s_offset["d"] > 0:
                ...
                latency_duration = min(
                    max(s_offset.get("t_lat", 0.0), 0.0), max(total_duration, 0.0)
                )
    ...
    # Add legend
    reconf_patch = patches.Patch(
        color=colors["reconfiguration"], label="Reconfiguration"
    )
    latency_patch = patches.Patch(color=colors["latency"], label="Latency overhead")
    trans_patch = patches.Patch(
        color=colors["transmission"], label="Payload transmission"
    )
    ...
    # Save figure when enabled. Format is inferred from filename extension.
    if save_as_pdf:
