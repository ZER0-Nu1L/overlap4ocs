...
详见 [`docs/math_model.md`](docs/math_model.md) 的详细数学建模。
...
overlap4ocs/
├── .github/
│   └── workflows/               # CI 冒烟 / repro-lite / repro-full
├── main.py                      # 主入口
├── config/
│   ├── instance.toml           # 问题实例参数
│   ├── program.toml            # 运行时配置
│   ├── program_png.toml        # 固定文件名 PNG 导出配置
...
├── docs/
│   ├── math_model.md           # 数学建模
│   ├── exp1.3-a2a-time-limit-study.md  # 求解时限研究说明
│   └── reproducibility.md      # 可复现流程
├── CLAUDE.md                   # 开发指南
└── README.md                   # 英文说明文档
```
...
# 说明：exp1.3 的聚合 CSV 会复用兼容的 exp1.1 基线：
# - exp1.3_ar: exp1.3 recursive-doubling + exp1.1 halving-doubling + 解析型 AR 基线
# - exp1.3_a2a_256: exp1.3 pairwise + exp1.1 bruck
# （复用行会按相同 topology/runtime 过滤）
# 这样可以减少重复计算，同时保持跨图对比口径一致。

# 3a) 使用 CLI 直接出图（exp1.1/1.2/1.3 + exp2.1/2.2）
uv run python scripts/simulation_fig.py --write-summary --output-dir figures/paper

# 注意：该命令会覆盖 figures/paper 下同名文件。
# 如果这里有你手动上传/整理的论文图，请改用其它目录：
